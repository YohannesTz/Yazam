...
Java_com_github_yohannestz_yazam_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
    std::string hello = "Message from JNI: you might have to malloc() my ass at some point ;)";
    return env->NewStringUTF(hello.c_str());
}

std::vector<std::vector<float>>
stride_windows(const std::vector<float> &data, size_t blocksize, size_t overlap) {
...
vector<pair<int, int>> detect_peaks(cv::Mat data) {
    ...
    cv::Mat tmpkernel = cv::getStructuringElement(cv::MORPH_CROSS, cv::Size(3, 3),
                                                  cv::Point(-1, -1));
    cv::Mat kernel = cv::Mat(PEAK_NEIGHBORHOOD_SIZE * 2 + 1, PEAK_NEIGHBORHOOD_SIZE * 2 + 1, CV_8U,
                             uint8_t(0));
